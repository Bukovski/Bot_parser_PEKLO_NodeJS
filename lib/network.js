...
network.update = () => {
  const xmlUpdate = `<update ${ dataUser }/>`;
  
  return network.address(xmlUpdate, (err) => {
    if (err) return console.error(err);
  });
};

network.auctionInfo = () => {
  const xmlAuctionInfo = `<get_auction_info ${ dataUser }/>`;
  
  return network.address(xmlAuctionInfo, (err) => {
    if (err) return console.error(err);
  });
};
